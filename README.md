...
The entire platform is **four components working together**. A Python/Flask backend manages device state and orchestrates actions. A React frontend provides the dashboard and visual editors. A Kotlin agent app runs on each Android device to report metrics and accept commands. And a Python library (droidlink) gives you programmatic access for scripts and CI pipelines.
...
- `pytest-droidlink` plugin with `device` and `device_pool` fixtures
...
├── droidlink/          Python library for direct device control
