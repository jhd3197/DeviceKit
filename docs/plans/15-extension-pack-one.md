...
**Status:** ✅ complete — all five phases shipped (browser, explorer, notification-capture + platform seams, registry, docs). Backend 197 tests pass; frontend builds. **Headline feature live-verified full-stack on a real phone**: against a running backend, installed `devicekit-browser` via the marketplace route, then over HTTP `goto`→example.com, `content`(title)="Example Domain", `evaluate`=42, pool `fetch` (round-robin, served by the device + cursor advanced), `screenshot`→valid PNG, session close. That E2E surfaced and fixed a real bug — `_resolve_serial` had called `devices.get()`, which returns a uiautomator2 *connection object* (not a dict) and would crash on hardware; it now validates via the lightweight `get_connected_devices()`. Live agent-dependent paths (explorer file ops, notification capture) are verified structurally; a physical-agent bringup is the only remaining manual check.
...
**Phase 5 notes (as shipped).** All three extensions are in
`backend/devicekit/data/registry_index.json` as first-party bundled builtins alongside
webhook-notify, each with a **real** sha256 (not appium's zeros). For a bundled builtin the
registry sha256 is a *content-integrity reference* — bundled installs come from the local
`builtin-extensions/` tree, not a download, so it is a reproducible content hash (sorted
`(relpath, len, bytes)`), regenerated by the new `backend/scripts/compute_builtin_sha256.py`
(`--all`); webhook-notify got one too, for consistency. `tests/test_pack_registry.py` proves
the three are bundled+first-party with 64-hex non-zero sha256s and install through the
registry/marketplace path (`install_extension_from_registry` → blueprint mounts at
`/ext/<slug>`). `docs/EXTENSIONS.md` gained a **device-scoped API convention** section
(`/ext/<slug>/devices/<device_id>/…`), documented `device_control.forward()/remove_forward()`,
added `jobs`/`schedules`/`automation_templates` contribution-point docs, and dropped the stale
"`jobs`/`notify` raise NotImplementedError" claim (plans 05/06 shipped). Template automations
were delivered in phases 2 and 4; explorer is UI-only by design.
