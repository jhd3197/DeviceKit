...
`builtin-extensions/devicekit-webhook-notify/`. The three device-facing pack extensions —
`devicekit-browser`, `devicekit-explorer`, `devicekit-notification-capture` — appear as worked
examples for specific seams.

> **Companion docs.** This is the narrative guide. For lookup, use the
> [Manifest Reference](manifest-reference.md) (every `extension.json` field + its rule) and the
> [SDK Reference](sdk-reference.md) (every `devicekit_sdk` export + signature). New here? Start
> with the [First Extension Tutorial](tutorial.md) — scaffold, install, and see a step type render
> in ~10 minutes.
...
Every field, its validation rule, and its default is a table in the
**[Manifest Reference](manifest-reference.md)** — the lookup companion to this narrative. The
essentials: `name` / `display_name` / `version` are required and raise immediately if missing;
`name` must match `^[a-zA-Z0-9_-]+$`; every other shape problem is accumulated so you see all of
them at once. Unknown `contributions` kinds are tolerated for forward-compatibility. The error
message always points you back to `GET /extensions/manifest-spec` or this guide.
...
Every export, its exact signature, and the permission each `device_control` method requires is a
lookup table in the **[SDK Reference](sdk-reference.md)**. In brief, the surface is: `db` (the
persistence seam), `logger`, `config` (in-process view with secrets), `broadcast`, `devices`
(read-only fleet accessors), `device_control` (permission-gated — below), `register_step_type` /
`register_fql_field`, `ai` (the AI-tool binder), `jobs` (plan 05 background work), `notify`
(plan 06 bus), and `require_permission` / `permissions` / `PermissionDenied` (the gate). All
contribution registrations are tracked per-slug and torn down on disable/uninstall.
